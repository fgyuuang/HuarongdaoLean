...
export const modelFormalization = {
  paragraphs: [
    'Lean 4 中有十个有限棋子、二维自然数坐标、矩形形状和四个移动方向。状态只记录棋子的左上角位置，占用格由形状函数计算。边界与不重叠约束组成可计算的 valid 检查，命题形式的 ValidState 则供证明使用。',
    '移动先平移目标棋子，再检查新状态。通过检查的动作进入 legalMoves，经典初态的合法性可由 rfl 直接归约。对同形棋子的位置排序后得到规范状态键，后续的商空间、BFS 和最短性证明都建立在这套有限状态转移系统上。'
  ],
  anchors: [
